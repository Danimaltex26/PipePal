...
router.post("/", auth, upload.array("images", 4), async (req, res) => {
  try {
    ...
    imageContent.push({
      type: "text",
      text: `Analyze this plumbing photo. Analysis type hint: ${analysis_type || "general"}. Return your analysis as the specified JSON object.`,
    });

    var aiResult = await callClaude({
      feature: 'photo_diagnosis',
      systemPrompt: PLUMBING_ANALYSIS_SYSTEM_PROMPT,
    ...
    const { data: saved, error: saveError } = await supabaseService
      ...
      .insert({
        ...
        diagnosis: result.plain_english_summary,
