...
router.post("/query", auth, async (req, res) => {
  try {
    ...
    if (!matches.length) {
      const { data: bySystemType } = await supabaseService
        .from("pipe_reference")
        .select("*")
        .ilike("system_type", `%${searchTerm}%`)
        .limit(5);
      if (bySystemType && bySystemType.length) matches = bySystemType;
    }
...
router.get("/browse", auth, async (req, res) => {
  try {
    const { category, system_type } = req.query;
    let q = supabaseService.from("pipe_reference").select("*").order("query_count", { ascending: false }).limit(50);
    if (category) q = q.eq("category", category);
    if (system_type) q = q.ilike("system_type", `%${system_type}%`);
