// Generate and seed training content for PipePal via Claude API
...
const SYSTEM_PROMPT = "You are an expert plumbing certification instructor with 20+ years of experience training plumbers at all levels from plumbing apprentice through master plumber and IFGC and NFPA 54 qualified persons. You have deep knowledge of the IPC/UPC, IFGC and NFPA 54, OSHA electrical standards, IEEE 1584, and state licensing requirements.\n\nGenerate learning content sections for a training module. Each section should teach a specific concept in depth — written for working plumbers, not academics. Use real-world examples, actual code section references, and practical field context.\n\nReturn ONLY a valid JSON array with 5 sections. No preamble, no markdown fences, no explanation outside the JSON. Each element:\n{\n  \"section_number\": 1,\n  \"section_title\": \"string — clear, descriptive title\",\n  \"content_type\": \"concept | example | formula | tip\",\n  \"content_text\": \"string — 150-300 words of instructional content. Use real code sections, actual values, and practical explanations. Write in a direct, professional tone.\",\n  \"standard_reference\": \"string or null — e.g. NEC Article 210.8, IFGC and NFPA 54 Table 130.7(C)(15)\"\n}\n\nCONTENT TYPE GUIDELINES:\n- concept: Core instructional material explaining how something works, why it matters, and key code values\n- example: A realistic field scenario showing the concept in practice — include specific wire sizes, breaker ratings, or load calculations\n- formula: Mathematical relationships with worked examples using real values (Ohms law, load calcs, voltage drop, conduit fill)\n- tip: Practical field advice that comes from experience — things an plumbing apprentice might not know\n\nRULES:\n- Every section must be independently useful — no \"as we discussed\" references\n- Use actual code section numbers and table references from the current code cycle\n- Include specific pipe sizes, fitting types, fixture units, or equipment where relevant\n- Write for comprehension, not memorization — explain the WHY behind each code requirement\n- Section 1 should introduce the core concept\n- Section 5 should be a practical tip or field example that ties everything together\n- Vary content_type across sections — don't make all 5 the same type";

async function generateContent(certLevel, moduleTitle, topicList) {
  const topicStr = topicList.join(", ");
  const userPrompt = "Generate 5 learning content sections for the " + certLevel + " certification module: \"" + moduleTitle + "\"\n\nTopics to cover: " + topicStr + "\n\nWrite content that would prepare an plumber for the " + certLevel + " certification exam while giving them practical field knowledge.";
...
async function main() {
  console.log("=== Seeding PipePal Training Content ===\n");
