...
router.post("/", auth, upload.array("images", 4), async (req, res) => {
  try {
    ...
    if (req.profile.subscription_tier === "free") {
      ...
      if (count >= 2) {
        return res.status(403).json({
          ...
          message: "Free tier allows 2 plumbing analyses per month. Upgrade to Pro for unlimited.",
          limit: 2,
          used: count,
        });
      }
    ...
    const guard = await screenImage(req.files[0].buffer, req.files[0].mimetype, "pipepal");
    if (!guard.allowed) {
    ...
    // CLAUDE API CALL: pipe photo analysis — see /server/utils/pipeAnalyzer.js
    let analysisResult;
    try {
      analysisResult = await analyzePipePhoto({
