...
export default function Paywall() {
  ...
  const packages = offerings?.all?.['PipePal']?.availablePackages || [];
  ...
  return (
    <div className="card" style={{ padding: '1.5rem' }}>
      ...
      <div style={{ display: 'flex', gap: '0.75rem', marginBottom: '1.25rem' }}>
        {annualPkg && <button onClick={() => setSelected('Annual')} style={{ flex: 1, padding: '1rem', borderRadius: 12, border: selected === 'Annual' ? '2px solid #3B82F6' : '2px solid #2A2A2E', background: selected === 'Annual' ? 'rgba(59,130,246, 0.08)' : 'transparent', cursor: 'pointer', textAlign: 'center', position: 'relative' }}>
          {savingsPercent > 0 && <span style={{ position: 'absolute', top: -10, right: 8, fontSize: '0.6875rem', fontWeight: 700, padding: '2px 8px', borderRadius: 8, background: '#3B82F6', color: '#0D0D0F' }}>Save {savingsPercent}%</span>}
          <p style={{ fontWeight: 700, fontSize: '1.125rem' }}>Annual</p>
          <p style={{ fontSize: '1.25rem', fontWeight: 700, color: '#3B82F6', margin: '0.25rem 0' }}>${annualCost}<span style={{ fontSize: '0.75rem', fontWeight: 400, color: '#A0A0A8' }}>/yr</span></p>
          <p style={{ fontSize: '0.75rem', color: '#6B6B73' }}>${monthlyEquiv}/mo</p>
        </button>}
        {monthlyPkg && <button onClick={() => setSelected('Monthly')} style={{ flex: 1, padding: '1rem', borderRadius: 12, border: selected === 'Monthly' ? '2px solid #3B82F6' : '2px solid #2A2A2E', background: selected === 'Monthly' ? 'rgba(59,130,246, 0.08)' : 'transparent', cursor: 'pointer', textAlign: 'center' }}>
